...
from tevatron.data import TrainDatasetForERNIE, QPCollatorForERNIE, TrainDatasetForERNIEMulti, QPCollatorForERNIEMulti
from tevatron.modeling import ERNIEModel, ERNIEREModel, ERNIEMultiModel
from tevatron.trainer import ErnieTrainer
from tevatron.datasets import HFTrainDataset, HFTrainDatasetWithRE, HFTrainDatasetWithCluster
from knowledge_bert.tokenization import BertTokenizer

logger = logging.getLogger(__name__)


def awaitGPU(gpuID: int):
    gpus = GPUtil.getGPUs()
    logger.info(f"available gpus: {len(gpus)}")
    while gpus[gpuID].memoryFree < 7000:    # busy loop
        logger.info(f"current free memory:{gpus[gpuID].memoryFree}, waiting for gpu...")
        time.sleep(60)
        gpus = GPUtil.getGPUs()
    logger.info("have enough gpu memory, execute python script...")
...
def main():
    ...
    """ TODO：方案一: 将外部知识生成的实体向量改为可训练的embedding """
    ...
    train_dataset = HFTrainDatasetWithCluster(tokenizer=tokenizer, data_args=data_args,
                                              cache_dir=data_args.data_cache_dir or model_args.cache_dir)
    if training_args.local_rank > 0:
        print("Waiting for main process to perform the mapping")
        torch.distributed.barrier()
    train_dataset = TrainDatasetForERNIEMulti(data_args, train_dataset.process(), tokenizer)
    if training_args.local_rank == 0:
    ...
    trainer = trainer_cls(
        ...
        data_collator=QPCollatorForERNIEMulti(
            tokenizer,
