...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            ent_hidden_size: Size of the encoder layers and the pooler layer for entity embedding.
            layer_types: Distribution of vanilla transformer layer and information fusion layer in ELK model.
        """
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        mixed_query_layer = self.query(hidden_states)  # [bs, max_seq_length, hidden_size]
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)

        query_layer = self.transpose_for_scores(
            mixed_query_layer)  # [bs, num_attention_heads, max_seq_length, attention_head_size]
        ...
        attention_scores = attention_scores / math.sqrt(
            self.attention_head_size)  # [bs, num_attention_heads, max_seq_length, max_seq_length]
        ...
        context_layer = torch.matmul(attention_probs,
                                     value_layer)  # [bs, num_attention_heads, max_seq_length, attention_head_size]
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)  # [bs, max_seq_length, hidden_size]
...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        # print("no attention for entity")
        self.self = BertSelfAttention(config)
        self.output = BertSelfOutput(config)

        config_ent = copy.deepcopy(config)
        config_ent.hidden_size = config.ent_hidden_size
    ...
    def forward(self, input_tensor, attention_mask, input_tensor_ent, attention_mask_ent):
        # self_output/input_tensor:[bs, max_seq_length, 768]    attention_mask:[bs,1,1,max_seq_length]
        self_output = self.self(input_tensor, attention_mask)
        # self_output_ent/input_tensor_ent:[bs, max_seq_length, 100]    attention_mask_ent:[bs,1,1,max_seq_length]
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense_ent = nn.Linear(config.ent_hidden_size, config.intermediate_size)  # 100 -> 3072
    ...
    def forward(self, hidden_states, hidden_states_ent):
        hidden_states_ = self.dense(hidden_states)  # [bs, max_seq_length, 3072]
        hidden_states_ent_ = self.dense_ent(hidden_states_ent)  # [bs, max_seq_length, 3072]
        ...
        hidden_states = self.intermediate_act_fn(
            hidden_states_ + hidden_states_ent_)  # hidden_states是token emb和ent emb融合后的中间状态
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense_ent = nn.Linear(config.intermediate_size, config.ent_hidden_size)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.LayerNorm_ent = BertLayerNorm(config.ent_hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, hidden_states_, input_tensor, input_tensor_ent):
        hidden_states = self.dense(hidden_states_)  # hidden_states_是token emb和ent emb融合后的中间状态
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)  # 残差连接

        hidden_states_ent = self.dense_ent(hidden_states_)
        hidden_states_ent = self.dropout(hidden_states_ent)
        hidden_states_ent = self.LayerNorm_ent(hidden_states_ent + input_tensor_ent)  # 残差连接
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, hidden_states_ent, attention_mask_ent, ent_mask,
        ...
        all_encoder_layers = []
        ent_all_encoder_layers = []
        ent_mask = ent_mask.to(dtype=next(self.parameters()).dtype).unsqueeze(-1)  # [bs, max_seq_length, 1]
        # if self.training:
        ...
        for layer_module in self.layer:
            ...
            if output_all_encoded_layers:
                ...
                ent_all_encoder_layers.append(hidden_states_ent)
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
            ent_all_encoder_layers.append(hidden_states_ent)
        return all_encoder_layers, ent_all_encoder_layers
...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        # prefix为state_dict中参数的前缀bert, 如bert.embeddings.word_embeddings.weight, 而模型只需要embeddings.word_embeddings.weight
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None,
                output_all_encoded_layers=False, return_dict=True):
        # input_ids: 输入文本的token ids, e.g. [101, 7599, 7180, ...., 0, 0, 0, ...]     >>[bs, max_seq_length]
        # token_type_ids: 输入文本的segment ids, e.g. [0, 0, 0, ...]                     >>[bs, max_seq_length]
        # attention_mask: 输入文本的mask, e.g. [1, 1, 1, 1, ...,0, 0, 0, ...]            >>[bs, max_seq_length]
        # input_ent: 输入文本中的实体序列的embs, 和input_ids长度相同, 非实体位置用向量[0,0,0,...]来填充,
        #            实体位置用knowledge graph embedding(KGE)预编码好的实体向量  >>[bs, max_seq_length, ent_dim]
        # ent_mask: [1,...,0,1,0,...,0,1,0,...] 1表示该token为实体(注意[CLS]也标记为实体了)        >>[bs, max_seq_length]
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids)  # Bert输入编码, 即token embs + pos embs + seg embs
        encoded_layers, ent_encoded_layers = self.encoder(embedding_output,
        ...
        if not output_all_encoded_layers:
            ...
            ent_encoded_layers = ent_encoded_layers[-1]

        outputs = (encoded_layers, ent_encoded_layers)
        if return_dict:
            outputs = {"pooler_output": pooled_output}
            if output_all_encoded_layers:
                outputs["all_hidden_state"] = encoded_layers
                outputs["all_ent_hidden_state"] = ent_encoded_layers
            else:
                outputs["last_hidden_state"] = encoded_layers
                outputs["last_ent_hidden_state"] = ent_encoded_layers
        return outputs
