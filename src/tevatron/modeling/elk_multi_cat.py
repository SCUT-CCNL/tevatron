...
class ELKMultiPooler(EncoderPooler):
    def __init__(self, input_dim: int = 768, output_dim: int = 768, tied=True):
        super(ELKMultiPooler, self).__init__()
...
class ELKMultiModel(nn.Module):
    ...
    def encode_passage(self, psg):
        ...
        psg_in = {'input_ids': psg['input_ids'], 'attention_mask': psg['attention_mask'], 'input_ent': psg['input_ent'],
                  'ent_mask': psg['ent_mask']}
        psg_out = self.lm_p(**psg_in, return_dict=True)
        ...
        p_ent_reps_att = dot_attention(p_reps.unsqueeze(dim=1), p_ent_hidden, p_ent_hidden, p_ent_bias.unsqueeze(dim=1))

        psg["ent_cluster_masks"] = psg["ent_cluster_masks"].float()
        p_ent_reps_cluster = torch.matmul(psg["ent_cluster_masks"], p_ent_hidden)
        mask_num = psg["ent_cluster_masks"].sum(dim=-1).unsqueeze(dim=-1)
        p_ent_reps_cluster = p_ent_reps_cluster / (mask_num + 1e-10)

        p_ent_reps_cmb = torch.concat((p_ent_reps_att, p_ent_reps_cluster), dim=1)
        p_reps = p_reps.unsqueeze(dim=1).repeat(1, p_ent_reps_cmb.shape[1], 1)
        p_reps_with_ent = torch.concat((p_reps, p_ent_reps_cmb), dim=-1)
    ...
    def compute_similarity(self, q_reps, p_reps):
        q_reps = q_reps.unsqueeze(dim=1).unsqueeze(dim=1)
        p_reps = p_reps.transpose(1, 2)
        scores = torch.matmul(q_reps, p_reps).max(-1).values
