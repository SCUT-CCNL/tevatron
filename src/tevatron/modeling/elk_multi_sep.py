...
class ELKMultiModel(nn.Module):
    ...
    def encode_passage(self, psg):
        ...
        p_ent_cluster_bias = (psg["ent_cluster_masks"] - 1) * 999999999.0
        cls_reps = p_reps.unsqueeze(dim=1).repeat(1, psg["ent_cluster_masks"].shape[1], 1)
        p_ent_reps_cluster = dot_attention(cls_reps, p_hidden, p_hidden, p_ent_cluster_bias)

        p_reps = p_reps.unsqueeze(dim=1)
        p_reps_with_ent = torch.concat((p_reps, p_ent_reps_cluster), dim=1)
