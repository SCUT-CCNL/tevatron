...
from .retriever import MultiVecFaissIPRetriever, BaseFaissIPRetriever
...
def write_ranking(corpus_indices, corpus_scores, q_lookup, ranking_save_file):
    if not os.path.exists(os.path.dirname(ranking_save_file)):
        os.makedirs(os.path.dirname(ranking_save_file))
...
def main():
    ...
    logger.info(args.passage_reps)
    if not os.path.exists(args.passage_reps):
        logger.info(f"{args.passage_reps} not exist...")
    logger.info(f'Pattern match found {len(index_files)} files; loading them into index.')

    p_reps_0, p_lookup_0 = pickle_load(index_files[0])
    assert p_reps_0.shape[0] == len(p_lookup_0)
    logger.info(p_reps_0.shape)
    ...
    # deprecated！！！
    # retriever = MultiVecFaissIPRetriever(p_reps_0)
    # shards = chain([(p_reps_0, p_lookup_0)], map(pickle_load, index_files[1:]))
    # if len(index_files) > 1:
    #     shards = tqdm(shards, desc='Loading shards into index', total=len(index_files))
    # look_up = []
    ...
    #     # p_reps: [num, m, dim]
    #     m = p_reps.shape[1]
    #     p_reps = p_reps.reshape(p_reps.shape[0] * m, p_reps.shape[2])
    #     retriever.add(p_reps)
    #     p_lookup = np.array(p_lookup).repeat(m)
    #     look_up += list(p_lookup)
